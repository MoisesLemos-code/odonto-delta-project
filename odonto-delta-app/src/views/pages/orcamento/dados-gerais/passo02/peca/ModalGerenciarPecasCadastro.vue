    ...
    <v-dialog v-model="value" max-width="700" scrollable persistent>
        <v-card elevation="4" tile>
            <v-toolbar dark color="primary" elevation="0">
                <v-toolbar-title>Cadastro de Peça</v-toolbar-title>
                <v-spacer/>
...
export default {
    name: 'ModalGerenciarPecasCadastro',
    props: {
        value: Boolean,
    },
    data() {
    ...
    methods: {
        ...mapActions([
            actionTypes.ORCAMENTO.PECA.CADASTRAR_PECA
        ]),
        ...
        async tratarEventoSalvar() {
            if (await this.validarDadosFormulario()) {
                ...
                await this.cadastrarPeca(this.dadosGerais)
