<template>
  <div class="aside-menu">
    <v-list subheader>
      <v-list-item two-line @click="editarUsuario" style="cursor: pointer">
        <v-list-item-avatar :class="{'ml-5' : !asideClosed}">
          <v-icon size='50'>mdi-account-circle</v-icon>
        </v-list-item-avatar>

        <v-list-item-content>
          <v-list-item-title>{{ usuarioLogado.nomeCompleto }}</v-list-item-title>
          <v-list-item-subtitle v-if="usuarioLogado.admin" class="text-perfil-admin">Administrador</v-list-item-subtitle>
          <v-list-item-subtitle v-else class="text-perfil-padrao">Padrão</v-list-item-subtitle>
        </v-list-item-content>
      </v-list-item>
      <v-divider></v-divider>
      <div v-for="menu in menuActions" :key="menu.name">
        ...
        <v-list-group v-else v-model="menu.expanded" no-action :prepend-icon="menu.icon" class="menu-item"
          ...
          <div class="aside-submenu" v-if="!asideClosed">
            <v-list-item
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            editarUsuario(){
                this.$router.push({name: 'MeuUsuario'})
            },
            open() {
