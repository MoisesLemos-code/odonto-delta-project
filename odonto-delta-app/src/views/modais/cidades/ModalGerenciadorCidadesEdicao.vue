<template>
    <div>
        <confirm-modal
                ...
                :persistencia="false"
                mensagemInicial="Deseja realmente excluir a cidade "
                :mensagemMeio="item.nomeSimples"
                mensagemFinal="?"/>
        <v-dialog v-model="value" max-width="700" scrollable persistent>
            <v-card elevation="4" tile>
                <v-toolbar dark color="primary" elevation="0">
                    <v-toolbar-title>Editando a cidade <b>{{item.nomeSimples}}</b></v-toolbar-title>
                    <v-spacer/>
                    ...
                    <v-container fluid grid-list-xl white>
                        <v-row wrap align-center white class="pl-10 pr-10">
                            <v-col cols="12" md="6" sm="6" xs="12">
                                <v-text-field
                                        v-model="dadosGerais.nomeSimples"
                                        name="nome"
...
<script>
    ...
    export default {
        name: 'ModalGerenciarCidadesEdicao',
        props: {
            value: Boolean,
            item: Object
        },
        ...
        methods: {
            ...mapActions([
                ...
                actionTypes.CADASTROS.CIDADE.EDITAR_CIDADE,
                actionTypes.CADASTROS.CIDADE.EXCLUIR_CIDADE
            ]),
            setarDadosGerais() {
            ...
            async tratarEventoExcluir() {
                await this.excluirCidade(this.dadosGerais.id)
            ...
            async tratarEventoSalvar() {
                if (await this.validarDadosFormulario()) {
                    this.setMensagemLoading('Salvando alterações da cidade...')
                    const estado = {id: this.dadosGerais.estadoId}
                    const cidade = {id: this.dadosGerais.id, nome: this.dadosGerais.nomeSimples, estado}
                    await this.editarCidade(cidade)
                    this.mostrarNotificacaoSucessoDefault()
                }
            },
            async validarDadosFormulario() {
                await this.$validator._base.validateAll()
                if (this.dadosGerais.nomeSimples && this.dadosGerais.estadoId) return true
                return false
            },
        }
    }
