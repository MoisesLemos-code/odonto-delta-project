    ...
    <v-layout column style="height: 50vh">
      <v-flex style="overflow-y: auto">
        <v-data-table
            ...
            :loading="false"
            no-data-text="Não há cidades cadastradas"
            class="table-list"
            hide-default-footer>
          <template v-slot:item.nomeSimples="{item}">
            <span class="d-inline-block text-truncate max-20">{{ item.nomeSimples | textoSemValor }}</span>
          </template>
          <template v-slot:item.estadoSigla="{item}">
            <span class="d-inline-block text-truncate max-15">{{ item.estadoSigla | textoSemValor }}</span>
          </template>
...
<script>
    ...
    export default {
        name: 'ModalGerenciarCidadesTabela',
        ...
        methods: {
            ...
            resetaPage() {
                this.$store.commit(mutationTypes.CADASTROS.CIDADE.RESETA_PAGE)
            },
        },
        watch: {
            paginacaoInterna: {
