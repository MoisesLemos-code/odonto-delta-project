...
public interface CobrancaController {
    ...
    ResponseEntity<AtualizarCobrancaOutput> atualizar(@PathVariable Long cobrancaId, @RequestBody @Valid AtualizarCobrancaInput input);

    @ApiOperation(value = "Estorna uma Cobrança")
    @PutMapping(value = "/estornar/{cobrancaId}")
    ResponseEntity<EstornarCobrancaOutput> estornar(@PathVariable Long cobrancaId);
